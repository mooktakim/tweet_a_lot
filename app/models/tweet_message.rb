class TweetMessage
  ...
  belongs_to :orig_tweet, :class_name => "TweetMessage"
  ...
  after_save do
    ...
    unless player = Player.first(:twitter_id => u.twitter_id)
  ...
  def self.create_from_hash(hash)
    if hash[:retweeted_status]
      orig_tweet = TweetMessage.first(:twitter_id => hash[:retweeted_status][:id])
      return nil unless orig_tweet # if tweet that was retweeted isn't in db, just ignore it
    end

    t = TweetMessage.new(:twitter_id => hash[:id])
    t.message = hash[:text]
