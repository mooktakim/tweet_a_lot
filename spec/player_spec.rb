...
describe "TweetALot::Player" do

  before do
    TweetALot::TweetMessage.destroy_all
    TweetALot::Player.destroy_all
  ...
  it "should create player with score 1 when new tweet is received" do
    tweet = @tweet
    msg = TweetALot::TweetMessage.create_from_hash(tweet)
    msg.twitter_id.should == tweet[:id]
    msg.user.should_not be_nil
    msg.urls.should be_empty

    player = TweetALot::Player.first(:twitter_id => tweet[:user][:id])
  ...
  it "should add 10 points to player if someone retweets" do
    ...
    retweet = @retweet
    msg = TweetALot::TweetMessage.create_from_hash(retweet)
